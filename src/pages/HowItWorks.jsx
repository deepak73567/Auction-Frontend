...
const HowItWorks = () => {
  ...
  return (
    <>
     ...
     <h1 className='text-[#D8125B] text-2xl font-bold mb-2 min-[480px]:text-4xl md:text-6xl xl:text-7xl 2xl:text-8xl '>Discover How PrimeBid Operates</h1>
     <div className='flex flex-col gap-4 my-5'>
{
    steps.map((element,index)=>{
        return(
            <div key={index} className='bg-[#fff] rounded-md p-2 lg:p-5 flex flex-col gap-2 group hover:bg-black transition-all duration-300'>
                <div className='bg-black text-white p-3 text-xl rounded-full w-fit group-hover:bg-[#d6482b] transition-all duration-300'>
                    {element.icon}
                </div>
                <h3 className={`text-[#106EBE] text-xl font-semibold mb-2 min-[480px]:text-xl md:text-2xl lg:text-3xl`}>{element.title}</h3>
